jQuery(document).ready(function($) {
  ...
  $('#search input').keyup(function() {
    ...
    if (text_val.match(regexp)) {
        ...
        if (!/\S/.test(text_val)) {} else {
          ...
          if (text_val.length >= 3) {
            ...
            ourReqest.onload = function() {
              ...
              if (counter == 0) {
                $('#city_load_more_out').empty();
                $('#city_load_more_out').append('<p class="sory_no_match">Es tut uns leid, es gibt keine Übereinstimmungen in den Suchergebnissen.</p>');
              }
      ...
      $('#city_load_more_out').show();
      $('#city_load_more_out').empty();
      $('#city_load_more_out').append('<p class="sory_no_match">Ein nicht-deutsches Tastaturlayout ist ausgewählt.</p>');
    }
  });
});
