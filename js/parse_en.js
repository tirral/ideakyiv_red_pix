jQuery(document).ready(function ($) {
  ...
  $("#search input").keyup(function () {
    ...
    if (text_val.match(regexp)) {
      if (text_val === "") {
        ...
        if (!/\S/.test(text_val)) {
          ...
          if (text_val.length >= 3) {
            ...
            ourReqest.open(
              "GET",
              "http://tigall.red-pix.com/wp-json/wp/v2/product?_embed&per_page=10&order=asc&search=" +
                searchField
            );
            ourReqest.onload = function () {
              ...
              if (counter == 0) {
                ...
                $("#city_load_more_out").append(
                  '<p class="sory_no_match">Sorry, there are no matches in the search results.</p>'
                );
              }
      ...
      $("#city_load_more_out").empty();
      $("#city_load_more_out").append(
        '<p class="sory_no_match">A non-English keyboard layout is selected.</p>'
      );
    }
  });
});
